<template>
  <main>
    <lazy-car-user-header title="Betlist Sports" icon="ticket-overview"> </lazy-car-user-header>
            ...
            <div>
              <div class="types date">
                ...
                <a-select :default-value="gameTypes[0].slug" class="text-sm">
                  <a-select-option class="text-primary-darker" v-for="type in gameTypes" :key="type.id" :value="type.slug">
                    {{ type.name }}
                  </a-select-option>
                </a-select>
              </div>
    ...
    <div class="px-4" v-if="getBets">
      ...
      <LazyBetBaseInput placeholder="Search Coupon Id" class="w-2/4 mb-3 md:w-1/4" keyup="getBetId" />
      ...
      <!-- <lazy-car-skeleton-table :columns="columns"></lazy-car-skeleton-table> -->
    </div>
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      if (this.getBets !== undefined) {
        return this.getBets.data.filter((row) => {
          const coupon = row.ticketId.toString().toLowerCase();
          const status = row.status.toString().toLowerCase();
          const user = row.bettor.toString().toLowerCase();
          const searchTerm = this.filterText.toLowerCase();
          return coupon.includes(searchTerm) || status.includes(searchTerm) || user.includes(searchTerm);
        });
      }
    },
  },
  data() {
    return {
      ...
      activeKey: ['1', '2'],
      columns: [
        { id: 1, name: 'ticketId', dataIndex: 'ticketId', key: 'ticketId', title: 'Coupon Id' },
        { id: 2, name: 'createdAt', dataIndex: 'createdAt', key: 'createdAt', title: 'Date', scopedSlots: { customRender: 'createdAt' } },
        { id: 3, name: 'date-of-result', dataIndex: 'date', key: 'date', title: 'Date Of Result', scopedSlots: { customRender: 'date' } },
        { id: 4, name: 'betslip', dataIndex: 'betslip', key: 'betslip', title: 'Amount', scopedSlots: { customRender: 'betslip' } },
        { id: 5, name: 'status', dataIndex: 'status', key: 'status', title: 'Status' },
        {
          id: 6,
          name: 'confirmedWinning',
          dataIndex: 'confirmedWinning',
          key: 'confirmedWinning',
          title: 'Winning',
          scopedSlots: { customRender: 'confirmedWinning' },
        },
        { id: 7, name: 'bettor', dataIndex: 'bettor', key: 'bettor', title: 'User' },
        { id: 8, name: 'show', dataIndex: 'show', key: 'show', title: '', scopedSlots: { customRender: 'show' } },
      ],
      periods: [
        {
          id: 1,
  ...
  created() {
    this.startFrom = moment().startOf('day').format('YYYY-MM-DD');
    this.startTo = moment().endOf('day').format('YYYY-MM-DD');
  },
  ...
  methods: {
    ...
    async fetchTableData() {
      ...
      let res = await this.$axios.$get(
        `/api/v1/sportbook/pool/bas/bets/search?username=${this.loggedInUser.username}&startDate=${this.query.startDate}&endDate=${this.query.endDate}`,
      );
  ...
  mounted() {
    this.$nextTick(() => {
      this.getBetApi({query: {tenBetUserId: this.loggedInUser.tenBetUserId}});
    });
  },
};
</script>
