    ...
    <div class="layout-container">
      <div class="layout-body__1">
        <div class="body-container">
          <div class="body-logo">
            <span class="text-xl icon-size"> Car </span>
          </div>
          <div class="body-main">
            <h1 class="text-2xl font-medium leading-10 text-primary-default-main">Sign Up</h1>

            <form class="mt-6 form-container" @submit.prevent="loginForm">
              <div>
                <LazyCarBaseInput label="FirstN ame" placeholder="First Name" size="small" />
                <!-- <small class="text-xs text-red-600" v-if="!$v.login.firstName.minLength"
                  >First Name must have at least {{ $v.login.firstName.$params.minLength.min }} letters.
                </small>
                <template v-if="$v.login.firstName.$error">
                  <small class="text-xs text-red-600" v-if="!$v.login.firstName.required">Username is required</small>
                </template> -->
              </div>
              <div>
                <LazyCarBaseInput label="Last Name" placeholder="Last Name" size="small" />
                <!-- <small class="text-xs text-red-600" v-if="!$v.login.lastName.minLength"
                  >Last Name must have at least {{ $v.login.lastName.$params.minLength.min }} letters.
                </small>
                <template v-if="$v.login.lastName.$error">
                  <small class="text-xs text-red-600" v-if="!$v.login.lastName.required">Username is required</small>
                </template> -->
              </div>
              <div>
                <LazyCarBaseInput label="Username" placeholder="Username" size="small" />
              ...
              <div>
                <LazyCarBaseInput label="Email" placeholder="Email" size="small" />
                <!-- <small class="text-xs text-red-600" v-if="!$v.login.email.minLength"
                  >Email must have at least {{ $v.login.email.$params.minLength.min }} letters.
                </small>
                <template v-if="$v.login.email.$error">
                  <small class="text-xs text-red-600" v-if="!$v.login.email.required">Email is required</small>
                </template> -->
              </div>
              <div>
          ...
          <div class="text-center body-footer">
            <span class="create-account">
              <p>
                ...
                <nuxt-link class="footer-link text-primary-default-main" :to="{ path: '/' }"> Sign In </nuxt-link>
              </p>
            </span>
          </div>
        </div>
